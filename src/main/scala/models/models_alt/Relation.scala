...
trait ValidRelation[S] { type Out <: Relation[_] ; def apply(dataset: List[S]): Out }
object ValidRelation {
    def apply[S](implicit ok: ValidRelation[S]): Aux[S, ok.Out] = ok
    type Aux[S, Out0 <: Relation[_]] = ValidRelation[S] { type Out = Out0 }
    protected def inhabit_Type[S, Repr0 <: HList](f: S => Repr0): Aux[S, Relation.Aux[S, Repr0]] = new ValidRelation[S] { type Out = Relation.Aux[S, Repr0] ; def apply(dataset: List[S]) = new Relation[S](dataset) { type Repr = Repr0 ; def toRepr(s: S) = f(s) } }
