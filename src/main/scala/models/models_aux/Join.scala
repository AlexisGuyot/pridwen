...
trait Join[LS <: HList, RS <: HList, LP, RP, M <: JoinMode] { type Out <: HList ; def apply(ldataset: List[LS], rdataset: List[RS]): List[Out] ; def apply2[K](ldataset: Map[K, List[LS]], rdataset: Map[K, List[RS]]): List[Out] }
object Join {
    ...
    private def inhabit_Type[LS <: HList, RS <: HList, LP, RP, M <: JoinMode, Out0 <: HList](
        ...
        = new Join[LS, RS, LP, RP, M] {
            ...
            def apply2[K](ldataset: Map[K, List[LS]], rdataset: Map[K, List[RS]]) = {
