...
object construct2 {
    def construct_from[ModelIn <: Model](from: ModelIn) = new {
        import constructor.ConstructReturn
        def a[ModelOut <: Model](implicit cons: ConstructReturn[ModelIn, ModelOut]): cons.Out = cons(from)
    }

    object constructor {
        trait ConstructReturn[ModelIn <: Model, ModelOut <: Model] { type Out ; def apply(from: ModelIn): Out }
        trait LowPriorityConstructReturn {
            type Aux[ModelIn <: Model, ModelOut <: Model, Out0] = ConstructReturn[ModelIn, ModelOut] { type Out = Out0 }
            implicit def default_rule[ModelIn <: Model, ModelOut <: Model]: ConstructReturn[ModelIn, ModelOut] = new ConstructReturn[ModelIn, ModelOut] { type Out = DefaultConstructor[ModelIn, ModelOut] ; def apply(from: ModelIn): Out = new DefaultConstructor[ModelIn, ModelOut](from) }
        }
        object ConstructReturn extends LowPriorityConstructReturn {
            implicit def model_out_is_graph[ModelIn <: Model, ModelOut <: Graph]: ConstructReturn[ModelIn, ModelOut] = new ConstructReturn[ModelIn, ModelOut] { type Out = GraphConstructor[ModelIn] ; def apply(from: ModelIn): Out = new GraphConstructor(from) }
        }

        class GraphConstructor[ModelIn <: Model](protected val from: ModelIn) {
            def withID [
                    Path_To_Source_ID <: HList, Path_To_Dest_ID <: HList, 
        ...
        class DefaultConstructor[ModelIn <: Model, ModelOut <: Model](protected val from: ModelIn) {
